...
% % save feynman diagram as a separate pdf
...
% Need: why is the state of the art not good enough?
  ...
  in insulators, in which beam-induced electronic excitations can reduce the
...
% Task / Document: what have we/this document done to address the need?
  ...
  electronic excitations in 2D insulating crystals by coupling quantum
  electrodynamics (QED) scattering amplitudes to density functional theory
  (DFT) single-particle orbitals.
...
beam electron exceeds some critical energy $E_c$.
% As the mass of the nucleus is much larger than that of the electron, $E_c$ is
% always much greater than $E_d$.  In most ground state materials, the critical
% energy is predicted to be anywhere from 10s to 100s of keV.
Many computational models have been proposed to predict both $E_d$ and $E_c$ to
calculate electron beam-induced sputtering rates in 2D crystals \cite{Meyer2012,
Susi2016, Yoshimura2018, Susi2019}. However, the vast majority of current
methods focus solely on interactions between the beam electrons and material
nuclei, neglecting any coupling with the material’s electrons.  Thus, while
present-day models give reasonable predictions for conductors \cite{Meyer2012},
where electronic relaxation is rapid, they often vastly underestimate the
atomic displacement rates in insulators.  For example, the critical energy for
sputtering boron or nitrogen from hexagonal boron nitride (hBN) is predicted to
be 80 keV \cite{Kotakoski2010}.
...
enable sputtering for energies well-below the ground state $E_c$.
...
\begin{equation}
  ...
  \left[
    \frac{E_\text{max}}{E^2}
    - \beta(\pi\alpha+\beta)\frac{1}{E}
    + \pi\alpha\beta\sqrt{\frac{E_\text{max}}{E^3}}
  \right],
  \label{eq:MF}
\end{equation}
%
...
compactness.
The critical energy $E_c$ is then the beam energy for which $E_\text{max}=E_d$.
Therefore, the observation of sputtering at energies well below $E_c$ is
...
% and
% material nuclei, $E_d$ and thus $E_c$ remain unchanged.
% As such that the cross-section for beam energies below the initially
% calculated $E_c$ remain tiny.
...
\begin{figure}
  \centering
  \includegraphics[width=.7\textwidth]{images/triad.pdf}
  \caption{
...
%     \textit{initial} interaction with the beam.
  ...
  \item Beam electrons and material nuclei: the energy transferred to a
    ...
    cross-section for an energy transfer of $E$ from the beam electron to
...
$E_d$ depends strongly on $n_f$, the interactions with the material electrons
must be considered.
...
most prominently when $\epsilon_b < E_c$.
...
\caption{
    ...
    Feynman diagrams called the $t$-channel (left) and the $u$-channel (right).
...
In this case, the magnitude of one outgoing momentum is similar to
...
We can use the free particle scattering amplitude to determine the amplitude
for the scattering of two arbitrary electron states $\phi_1$ and $\phi_2$ into
$\phi_3$ and $\phi_4$.  This is obtained by sandwiching the scattering operator
between the initial and final 2-particle states, i.e.,
...
\end{equation}
%
We can use the delta function to integrate over $\mathbf{p}_4$ and $p_3^z$,
yielding
...
cross-sectional area of the simulated crystal, i.e., the volume and cross
sectional area of the unit cell times the number of k-points used to sample the
BZ.
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{images/exciProbs_overlap.pdf}
  \caption{
    Probabilities of initially exciting a certain number of electrons $n_i$ in
    hBN and MoS$_2$ with a beam electron.
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{./images/eigenvals_overlap.pdf}
  \caption{
...
$E_c\sim80$ keV \cite{Jin2009,Meyer2009,Kotakoski2010,Cretu2015}.
...
\begin{figure} 
  \centering
  \includegraphics[width=\textwidth]{images/edgeCross.pdf}
  \caption{
    ...
    excitation lifetime of $\tau=400$ fs (labelled ``exc.'' for excited).
...
agreement if the predicted excitation lifetime is set to $\tau \sim 400$ fs.
...
ground state $E_c\sim 90$ keV \cite{Kretschmer2020}.
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{images/MoS2Cross.pdf}
  \caption{
...
% Unlike figure \ref{fig:edgeCross}c, we do not sum over the cross
% sections of all excitation numbers $n_f$.
...
The cutoff energies for hBN and MoS$_2$ were set to 312 and 285 eV
respectively, while their cell-heights were respectively set to 17 and 12
{\AA}.
